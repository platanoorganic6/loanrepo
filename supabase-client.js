...
(function () {
  ...
  var DB = {
    ...
    // The report is drawn server-side with pdf-lib. html2canvas produced
    // entirely blank pages here: the design system's colour ramps are OKLCH,
    // which it does not support, so it failed silently on every capture.
    storeDiagnosisReport: function (loan) {
      if (!enabled) return Promise.resolve({ ok: false, reason: "not-configured" });
      return client.functions.invoke("report-assemble", { body: loan })
        .then(function (r) {
          var d = r.data || {};
          if (r.error || !d.ok) {
            return { ok: false, error: (r.error && r.error.message) || d.error || "report-failed" };
          }
