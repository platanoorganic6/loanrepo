...
(function(){
  ...
  var GUIDE_URL="./quiet-years.html?v=20260916-2";
  ...
  function wireNudges(){var b=Array.prototype.slice.call(document.querySelectorAll("button")).filter(function(x){return /BORROWER'S GUIDE|THE QUIET YEARS|THE BOOK/i.test(x.textContent||"")&&/299/.test(x.textContent||"")});b.slice(0,2).forEach(function(x,i){x.setAttribute("data-loanrepo-ebook-nudge",i?"second":"first");x.textContent=COPY[i?"second":"first"];x.setAttribute("aria-label",x.textContent)})}
  function beginGuide(){var d=snapshot();if(!d){alert("Please complete the diagnosis first.");return}set(PENDING,JSON.stringify({loanQuery:loanQuery(d),createdAt:new Date().toISOString()}));location.href=GUIDE_URL}
  function goAfterAuth(){if(!user)return;var target=null;if(get(PENDING)){del(PENDING);target=GUIDE_URL}else if(get(RETURN_PURCHASE)){del(RETURN_PURCHASE);target=GUIDE_URL}if(target)location.href=target}
